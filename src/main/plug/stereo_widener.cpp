...
#include <private/plugins/stereo_widener.h>
...
namespace lsp
{
    ...
    namespace plugins
    {
        ...
        {
            &meta::stereo_widener_mono,
            &meta::stereo_widener_stereo
        };

        static plug::Module *plugin_factory(const meta::plugin_t *meta)
        {
            return new stereo_widener(meta);
        }
        ...
        //---------------------------------------------------------------------
        // Implementation
        stereo_widener::stereo_widener(const meta::plugin_t *meta):
            Module(meta)
        {
        ...
        void stereo_widener::init(plug::IWrapper *wrapper, plug::IPort **ports)
        {
        ...
        void stereo_widener::update_sample_rate(long sr)
        {
            // Update sample rate for the bypass processors
            for (size_t i=0; i<nChannels; ++i)
            {
                channel_t *c    = &vChannels[i];
                c->sLine.init(dspu::millis_to_samples(sr, meta::stereo_widener::DELAY_OUT_MAX_TIME));
                c->sBypass.init(sr);
            }
        }

        void stereo_widener::update_settings()
        {
        ...
        void stereo_widener::process(size_t samples)
        {
        ...
        void stereo_widener::dump(dspu::IStateDumper *v) const
        {
